...
def display_name(app_class, app_title):
    m = re.match(r'^steam_app_(\d+)$', app_class.lower())
    if m:
        sn = resolve_steam_app_name(m.group(1))
        if sn:
            return sn
    return app_title or app_class
...
def main():
    ...
    for row in c.fetchall():
        ...
        all_apps.append({
            "class": app_class, "name": display_name(app_class, app_title), "icon": get_app_icon(app_class),
    ...
    for r in week_apps_rows:
        cls, title, secs = r
        week_apps.append({
            "class": cls, "name": display_name(cls, title), "icon": get_app_icon(cls),
