...
PORTAL_NOISE_PREFIXES = ("org.freedesktop.impl.portal", "xdg-desktop-portal")
PORTAL_NOISE_CLASSES = {"hyprland-share-picker", "org.freedesktop.impl.portal.desktop.kde"}

def is_noise_class(app_class):
    if not app_class:
        return False
    cls = app_class.lower()
    if cls in PORTAL_NOISE_CLASSES:
        return True
    for prefix in PORTAL_NOISE_PREFIXES:
        if cls.startswith(prefix):
            return True
    return False

def is_hidden_app(app_class):
    if not app_class:
        return True
    if app_class in SYSTEM_STATES:
        return True
    return is_noise_class(app_class)
...
def resolve_steam_app_name(appid):
    if appid in STEAM_NAME_CACHE:
        return STEAM_NAME_CACHE[appid]
    candidates = [
        os.path.expanduser("~/.local/share/Steam/steamapps/appmanifest_%s.acf" % appid),
        os.path.expanduser("~/.var/app/com.valvesoftware.Steam/.local/share/Steam/steamapps/appmanifest_%s.acf" % appid),
    ]
    for path in candidates:
        try:
            ...
            if m:
                STEAM_NAME_CACHE[appid] = m.group(1)
                return m.group(1)
        except OSError:
            continue
    STEAM_NAME_CACHE[appid] = None
    return None
...
def resolve_app_name(app_class, raw_title):
    ...
    steam_match = re.match(r'^steam_app_(\d+)$', app_class_lower)
    if steam_match:
        steam_name = resolve_steam_app_name(steam_match.group(1))
        if steam_name:
            DESKTOP_CACHE_NAME[app_class_lower] = steam_name
            return steam_name
...
def get_app_icon(app_class):
    ...
    steam_match = re.match(r'^steam_app_(\d+)$', app_class_lower)
    if steam_match:
        icon_name = "steam_icon_" + steam_match.group(1)
        icon_paths = [
            os.path.expanduser("~/.local/share/icons/hicolor/128x128/apps/%s.png" % icon_name),
            os.path.expanduser("~/.local/share/icons/hicolor/64x64/apps/%s.png" % icon_name),
        ]
        if any(os.path.exists(p) for p in icon_paths):
            return icon_name
...
def discover_hypr_signature():
    sig = os.environ.get("HYPRLAND_INSTANCE_SIGNATURE")
    if sig:
        return sig
    for base in (os.environ.get("XDG_RUNTIME_DIR", "/tmp"), "/tmp"):
        root = os.path.join(base, "hypr")
        try:
            for entry in os.listdir(root):
                p = os.path.join(root, entry)
                if os.path.isdir(p) and os.path.exists(os.path.join(p, ".socket2.sock")):
...
class DaemonTracker:
    ...
    def full_sync(self, target_date):
        ...
        for row in c.fetchall():
            ...
            if is_hidden_app(app_class):
                continue
            visible_total += secs
            all_apps.append({
                "class": app_class, "name": resolve_app_name(app_class, app_title), "icon": get_app_icon(app_class),
                "seconds": secs, "percent": round((secs / visible_total) * 100, 1) if visible_total > 0 else 0
            })
        ...
        week_apps_visible = [r for r in week_apps_rows if not is_hidden_app(r[0])]
        week_apps_total = sum([r[2] for r in week_apps_visible])
        week_apps = []
        for r in week_apps_visible:
            cls, title, secs = r
            week_apps.append({
                "class": cls, "name": resolve_app_name(cls, title), "icon": get_app_icon(cls),
